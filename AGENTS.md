...
编写代码时，在合适的地方添加注释文档，例如方法、需要注意的地方等。
...
- 新增数据库字段时请统一使用驼峰命名，以便在应用层直接映射字段名称。例如 `players` 表中的
  `avatarColor` 字段，后续新增字段应保持同样的命名风格。

如需新增计分模板，请参阅 `docs/new_scoring_template_guide.md` 获取完整开发指引。
