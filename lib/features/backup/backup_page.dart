...
class _BackupPageState extends ConsumerState<BackupPage> {
  ...
  Future<void> _handleExport() async {
    try {
      // 1. 从 Provider 获取用户在UI上勾选的选项 (例如：是否包含数据库)。
      //    我们只传递业务选项，不传递任何关于路径或文件名的信息。
      final exportOptions = ref.read(exportOptionsManagerProvider);

      // 2. 直接调用 BackupService 的 exportData 方法。
      //    BackupService 内部现在会自己判断平台：
      //    - 如果是鸿蒙，它会自己生成临时文件并调用 saveFile。
      //    - 如果是其他平台，它会自己调用 GlobalExportConfigDialog 来获取路径。
